...
void handleAutoFeederPage() {
    ...
    html += "  if (button.disabled) return;\n";
    html += "  button.disabled = true;\n";
    html += "  setTimeout(() => { button.disabled = false; }, 3000);\n";
    html += "  fetch('/autofeeder/' + feederId + '/activate')\n";
    html += "    .then(response => {\n";
    html += "      if (!response.ok) {\n";
    html += "        return response.json().then(data => Promise.reject({status: response.status, data: data}));\n";
    html += "      }\n";
    ...
    html += "      } else {\n";
    html += "        let errorMsg = 'Ошибка активации';\n";
    html += "        if (data.error === 'not_initialized') errorMsg = 'Кормушка не инициализирована';\n";
    html += "        else if (data.error === 'already_running') errorMsg = 'Кормушка уже работает';\n";
    html += "        else if (data.error === 'limit_busy') errorMsg = 'Концевик занят, подождите';\n";
    html += "        else if (data.error === 'activation_failed') errorMsg = 'Не удалось активировать кормушку';\n";
    html += "        alert(errorMsg);\n";
    html += "      }\n";
    html += "    })\n";
    html += "    .catch(error => {\n";
    html += "      console.error('Ошибка активации кормушки:', error);\n";
    html += "      if (error.data && error.data.error) {\n";
    html += "        let errorMsg = 'Ошибка активации';\n";
    html += "        if (error.data.error === 'not_initialized') errorMsg = 'Кормушка не инициализирована';\n";
    html += "        else if (error.data.error === 'already_running') errorMsg = 'Кормушка уже работает';\n";
    html += "        else if (error.data.error === 'limit_busy') errorMsg = 'Концевик занят, подождите';\n";
    html += "        alert(errorMsg);\n";
    html += "      } else {\n";
    html += "        alert('Ошибка соединения с сервером');\n";
    html += "      }\n";
    html += "    });\n";
    html += "}\n";
    html += "\n";
