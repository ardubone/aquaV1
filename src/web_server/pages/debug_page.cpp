...
void handleDebugPage() {
    ...
    html += "<label for=\"tankLrgSelect\" class=\"form-label\">" + String(TANK_LRG_NAME) + ":</label>\n";
    html += "<select class=\"form-select\" id=\"tankLrgSelect\">\n";
    html += "<option value=\"\">Выберите датчик...</option>\n";
    html += "</select>\n";
    html += "</div>\n";
    html += "<div class=\"col-md-6 mb-3\">\n";
    html += "<label for=\"tankSmlSelect\" class=\"form-label\">" + String(TANK_SML_NAME) + ":</label>\n";
    html += "<select class=\"form-select\" id=\"tankSmlSelect\">\n";
    html += "<option value=\"\">Выберите датчик...</option>\n";
    html += "</select>\n";
    html += "</div>\n";
    html += "</div>\n";
    ...
    html += "      document.getElementById('tankLrgAddress').textContent = formatAddress(data.tankLrgAddress);\n";
    html += "      document.getElementById('tankSmlAddress').textContent = formatAddress(data.tankSmlAddress);\n";
    html += "      \n";
    ...
    html += "      const tankSmlSelect = document.getElementById('tankSmlSelect');\n";
    html += "      \n";
    html += "      tankLrgSelect.innerHTML = '<option value=\"\">Выберите датчик...</option>';\n";
    html += "      tankSmlSelect.innerHTML = '<option value=\"\">Выберите датчик...</option>';\n";
    html += "      \n";
    html += "      data.sensors.forEach(sensor => {\n";
    html += "        const addrStr = addressToString(sensor.address);\n";
    html += "        const optionLrg = document.createElement('option');\n";
    html += "        optionLrg.value = addrStr;\n";
    html += "        optionLrg.textContent = formatAddress(sensor.address) + ' (' + sensor.temp.toFixed(1) + '°C)';\n";
    html += "        if (addressToString(data.tankLrgAddress) === addrStr) optionLrg.selected = true;\n";
    html += "        tankLrgSelect.appendChild(optionLrg);\n";
    html += "        \n";
    html += "        const optionSml = document.createElement('option');\n";
    html += "        optionSml.value = addrStr;\n";
    html += "        optionSml.textContent = formatAddress(sensor.address) + ' (' + sensor.temp.toFixed(1) + '°C)';\n";
    html += "        if (addressToString(data.tankSmlAddress) === addrStr) optionSml.selected = true;\n";
    html += "        tankSmlSelect.appendChild(optionSml);\n";
    html += "      });\n";
    html += "      \n";
    ...
    html += "        const addrStr = addressToString(sensor.address);\n";
    html += "        let assignment = '';\n";
    html += "        if (addressToString(data.tankLrgAddress) === addrStr) assignment += '<span class=\"badge bg-primary\">" + String(TANK_LRG_NAME) + "</span> ';\n";
    html += "        if (addressToString(data.tankSmlAddress) === addrStr) assignment += '<span class=\"badge bg-success\">" + String(TANK_SML_NAME) + "</span>';\n";
    ...
    html += "  const tankLrgAddr = document.getElementById('tankLrgSelect').value;\n";
    html += "  const tankSmlAddr = document.getElementById('tankSmlSelect').value;\n";
    html += "  \n";
    html += "  if (!tankLrgAddr || !tankSmlAddr) {\n";
    html += "    alert('Выберите датчики для обоих резервуаров');\n";
    html += "    return;\n";
    html += "  }\n";
    html += "  \n";
    html += "  const formData = new URLSearchParams();\n";
    html += "  formData.append('tankLrg', tankLrgAddr);\n";
    html += "  formData.append('tankSml', tankSmlAddr);\n";
    html += "  \n";
