...
- На каждый мотор обязательно установить диод 1N5819 для защиты от обратной ЭДС (анод к Drain MOSFET, катод к +3.3V питания мотора)
...
2. Подключить провода напрямую к мотору:
   - Плюс мотора → через MOSFET IRLB8721 к +3.3V (через AMS1117)
   - Минус мотора → к GND
...
```
+5V ── AMS1117 ── +3.3V ──┬── Мотор (+) ── MOSFET Drain ── Мотор (-) ── MOSFET Source ── GND
                          │
                          └── Диод 1N5819 (катод) ── Диод анод ── MOSFET Drain
...
- Диод 1N5819 (анод к Drain, катод к +3.3V) защищает от обратной ЭДС мотора
